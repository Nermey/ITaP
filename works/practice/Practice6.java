...
public class Practice6 {
    ...
    public static String fractions(String num) { // task 6
        int startPow = num.indexOf("(") - num.indexOf(".") - 1; // часть от . до ( в которую нужно возвести 10 при случаях 1.1(2) -> 11.(2)
        int endPow = num.substring(num.indexOf("."), num.length() - 1).length() - 2; // часть от . до конца стоки, чтобы узнать степень 10 для выражения х
        int partBeforePeriod = Integer.parseInt(num.substring(0, num.indexOf("(")).replace(".", "")); // часть до периода 1.1(2) -> 11.(2)
        int newNum = Integer.parseInt(num.replace(".", "").replace("(", "").replace(")", "")); // число доведенное до целого 1.(123) -> 1123
       int difference = newNum - partBeforePeriod; // числитель
       int denominator = (int)(Math.pow(10, endPow) - Math.pow(10, startPow)); // разница между двумя уравнениями для х
